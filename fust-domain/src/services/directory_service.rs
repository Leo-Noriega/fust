...
impl DirectoryService {
    ...
    pub async fn create(&self, path: &Path) -> Result<(), FsError> {
        ...
        if let Some(invalid_char) = path.to_string_lossy().chars().find(|c| *c == '\0') {
            return Err(FsError::InvalidPath(format!(
                "Path contains invalid character: {invalid_char:?}"
            )));
        }
        ...
        match fs::create_dir_all(path).await {
            ...
            Err(e) => {
                ...
                match e.kind() {
                    ...
                    std::io::ErrorKind::NotFound => Err(FsError::NotFound(format!(
                        "Parent directory not found: {}",
                        path.display()
                    ))),
                    std::io::ErrorKind::InvalidInput => Err(FsError::InvalidPath(format!(
                        "Invalid path: {}",
                        path.display()
                    ))),
    ...
    pub async fn rename(&self, old_path: &Path, new_path: &Path) -> Result<(), FsError> {
        ...
        match fs::rename(old_path, new_path).await {
            Ok(()) => {
                info!(
                    "Successfully renamed directory from {:?} to {:?}",
                    old_path, new_path
                );
                Ok(())
            }
            Err(e) => {
                ...
                match e.kind() {
                    ...
                    std::io::ErrorKind::AlreadyExists => Err(FsError::Io(format!(
                        "Destination already exists: {}",
                        new_path.display()
                    ))),
                    _ => Err(FsError::Io(format!(
    ...
    pub async fn delete(&self, path: &Path) -> Result<(), FsError> {
        ...
        match fs::remove_dir_all(path).await {
            ...
            Err(e) => {
                ...
                match e.kind() {
                    std::io::ErrorKind::PermissionDenied => Err(FsError::PermissionDenied(
                        format!("Permission denied deleting directory: {}", path.display()),
                    )),
                    std::io::ErrorKind::NotFound => Err(FsError::NotFound(format!(
                        "Directory not found: {}",
                        path.display()
