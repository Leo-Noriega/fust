...
async fn test_directory_service_create_nested() {
    let service = DirectoryService::new();
    let temp_dir = TempDir::new().unwrap();
    let test_path = temp_dir.path().join("nested").join("deep").join("directory");
...
async fn test_directory_service_delete_nested() {
    ...
    service.create(&test_path.join("subdir1").join("subdir2")).await.unwrap();
    assert!(test_path.exists());
